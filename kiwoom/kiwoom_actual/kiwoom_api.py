...
class KiwoomAPI(QObject):
    ...
    def __init__(self):
        ...
        self.account_list = []
        self.ocx = None
        self.initialization_timeout = 10  # 10초 타임아웃
        
        # QApplication이 존재하는지 확인
        if QApplication.instance() is None:
            self.logger.warning("QApplication이 생성되지 않음 - 모의 모드로 실행")
            self.ocx = None
        elif KIWOOM_AVAILABLE:
            # 안전한 초기화 시도
            self.safe_initialize_ocx()
        
        if not KIWOOM_AVAILABLE or not self.ocx:
            self.logger.info("키움 API 사용 불가 - 모의 모드로 실행")
    
    def safe_initialize_ocx(self):
        """안전한 OCX 초기화"""
        try:
            self.logger.info("키움 OCX 컨트롤 생성 시도 중...")
            
            # 타임아웃 타이머 설정
            timeout_timer = QTimer()
            timeout_timer.setSingleShot(True)
            timeout_timer.timeout.connect(self.on_initialization_timeout)
            timeout_timer.start(self.initialization_timeout * 1000)
            
            # OCX 컨트롤 생성
            self.ocx = QAxWidget("KHOPENAPI.KHOpenAPICtrl.1")
            
            timeout_timer.stop()  # 성공하면 타이머 정지
            
            if self.ocx:
                self.logger.info("✓ 키움 OCX 컨트롤 생성 성공")
                
                # 시그널 연결 시도
                self.setup_signals()
                
                # OCX 상태 확인
                if self.check_ocx_status():
                    self.logger.info("✓ 키움 OCX 상태 정상")
                else:
                    self.logger.warning("⚠ 키움 OCX 상태 불안정 - 모의 모드로 전환")
                    self.ocx = None
            else:
                self.logger.warning("키움 OCX 컨트롤 생성 실패 - 모의 모드로 전환")
                
        except Exception as e:
            self.logger.warning(f"키움 OCX 초기화 실패: {e} - 모의 모드로 전환")
            self.ocx = None
    
    def on_initialization_timeout(self):
        """초기화 타임아웃 처리"""
        self.logger.warning(f"키움 OCX 초기화 타임아웃 ({self.initialization_timeout}초) - 모의 모드로 전환")
        self.ocx = None
    
    def check_ocx_status(self) -> bool:
        """OCX 상태 확인"""
        try:
            if not self.ocx:
                return False
            ...
            self.logger.info(f"키움 API 경로: {version}")
            return True
            
        except Exception as e:
            self.logger.error(f"OCX 상태 확인 실패: {e}")
            return False
    ...
    def setup_signals(self):
        ...
        try:
            ...
            # 안전한 시그널 연결
            signal_connections = [
                ('OnEventConnect', self.on_event_connect),
                ('OnReceiveTrData', self.on_receive_tr_data),
                ('OnReceiveChejanData', self.on_receive_chejan_data),
            ]
            
            connected_signals = 0
            for signal_name, handler in signal_connections:
                try:
                    if hasattr(self.ocx, signal_name):
                        signal = getattr(self.ocx, signal_name)
                        signal.connect(handler)
                        connected_signals += 1
                        self.logger.debug(f"✓ {signal_name} 시그널 연결 성공")
                    else:
                        self.logger.debug(f"⚠ {signal_name} 시그널 없음")
                except Exception as e:
                    self.logger.warning(f"✗ {signal_name} 시그널 연결 실패: {e}")
            
            if connected_signals > 0:
                self.logger.info(f"✓ 키움 API 시그널 연결 완료 ({connected_signals}개)")
            else:
    ...
    def initialize(self) -> bool:
        """API 초기화 (타임아웃 적용)"""
        try:
            self.logger.info("키움 API 초기화 시작...")
            
            # QApplication 존재 확인
            if QApplication.instance() is None:
                self.logger.error("QApplication이 생성되지 않음")
                return self.fallback_to_mock_mode()
            
            if not KIWOOM_AVAILABLE or not self.ocx:
                self.logger.info("키움 API 사용 불가 - 모의 모드로 전환")
                return self.fallback_to_mock_mode()
            ...
            try:
                ...
                connection_timer.timeout.connect(lambda: self.logger.warning("연결 요청 타임아웃"))
                connection_timer.start(8000)  # 8초 타임아웃
                ...
                if result == 0:
                    ...
                    # 연결 완료 대기 (최대 10초)
                    wait_count = 0
                    while not self.connected and wait_count < 100:  # 10초 대기
                        QApplication.processEvents()  # 이벤트 처리
                        time.sleep(0.1)
                        wait_count += 1
                    
                    if self.connected:
                        self.logger.info("✓ 키움 API 로그인 완료")
                        return True
                    else:
                        self.logger.warning("키움 API 로그인 타임아웃 - 모의 모드로 전환")
                        return self.fallback_to_mock_mode()
                
                else:
                    self.logger.error(f"키움 API 연결 실패 (코드: {result}) - 모의 모드로 전환")
                    return self.fallback_to_mock_mode()
            ...
            self.logger.error(f"API 초기화 오류: {e} - 모의 모드로 전환")
            return self.fallback_to_mock_mode()
    
    def fallback_to_mock_mode(self) -> bool:
        """모의 모드로 전환"""
        self.connected = True
    ...
    def is_connected(self) -> bool:
        ...
        try:
            ...
            actual_connected = (state == 1)
            
            if actual_connected != self.connected:
                self.logger.info(f"연결 상태 변경: {self.connected} -> {actual_connected}")
    ...
    def on_event_connect(self, err_code):
        ...
        try:
            if err_code == 0:
                ...
                error_msg = error_messages.get(err_code, f"알 수 없는 오류 (코드: {err_code})")
                self.logger.error(f"키움 API 로그인 실패: {error_msg}")
                self.login_completed.emit(False)
        except Exception as e:
            self.logger.error(f"로그인 이벤트 처리 오류: {e}")
    ...
    def get_account_list(self) -> List[str]:
        ...
        try:
            ...
            if accounts:
                account_list = accounts.split(';')[:-1]  # 마지막 빈 항목 제거
                self.logger.info(f"계좌 목록: {account_list}")
                return account_list
            else:
                return [Config.DEFAULT_ACCOUNT]
        except Exception as e:
            self.logger.error(f"계좌 목록 조회 오류: {e}")
            return [Config.DEFAULT_ACCOUNT]
    ...
    def get_stock_price(self, stock_code: str) -> Dict[str, Any]:
        ...
        try:
            if not KIWOOM_AVAILABLE or not self.ocx or Config.MOCK_MODE:
                # 모의 데이터 생성
                return self.generate_mock_stock_data(stock_code)
    ...
    def generate_mock_stock_data(self, stock_code: str) -> Dict[str, Any]:
        """모의 주식 데이터 생성"""
    ...
    def get_api_status(self) -> Dict[str, Any]:
        """API 상태 정보 반환"""
        return {
            'available': KIWOOM_AVAILABLE,
            'ocx_created': self.ocx is not None,
            'connected': self.connected,
            'account_count': len(self.account_list),
            'mock_mode': Config.MOCK_MODE or not self.ocx
        }
